...
function Solution({ questionId }) {
  ...
  const checkExistingSolution = async () => {
    try {
      const response = await api.get(`/api/questions/solution/${questionId}`);
      setSolutionExists(true);
      const contentDisposition = response.headers['content-disposition'];
      if (contentDisposition) {
        const match = contentDisposition.match(/filename="(.+)"/);
        if (match) setSolutionFileName(match[1]);
      }
    } catch (err) {
  ...
  const handleUploadSolution = async () => {
    ...
    try {
      ...
      await api[method](`/api/questions/solution/${questionId}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });

      const msg = solutionExists ? 'Solution updated.' : 'Solution uploaded.';
      setSuccess(msg);
      notify.success(msg);
      ...
      notify.error(err.message || 'Failed to upload solution');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadSolution = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to download solution');
      notify.error(err.message || 'Failed to download solution');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleViewSolution = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to load solution content');
      notify.error(err.message || 'Failed to load solution content');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm">
      ...
      {solutionExists && (
        <div className="border p-4 rounded-md mb-4 bg-gray-50">
          ...
          <div className="flex items-center justify-between">
            <span className="text-sm text-gray-600">📄 {solutionFileName || 'Solution file'}</span>
      ...
      <div className="space-y-4">
        <h3 className="font-medium">{solutionExists ? 'Update Solution' : 'Upload Solution'}</h3>
        ...
        <div className="flex space-x-2 justify-end">
          ...
          >
            {loading ? 'Processing...' : solutionExists ? 'Update Solution' : 'Upload Solution'}
          </button>
        </div>
      </div>
    </div>
